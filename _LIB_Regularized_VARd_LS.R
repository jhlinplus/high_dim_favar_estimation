...
regularized_VAR_est = function(Y=NULL,X,d=NULL,lambda,penalty.factor=NULL,alpha=1,refit=FALSE,parallel=FALSE)
{
	...
	{
    	if (is.null(d))
			stop("Error in sparse_VAR_est(): response matrix is not specified. d must be non NULL.\n");
    ...
    if ( (d*p)!= ncol(A) )
    {
        warning("The number of columns of A does not equal to dim(X) * d.\n");
    }
...
regularized_VAR_bic = function(Y=NULL,X,d=NULL,lambda_seq=NULL,penalty.factor=NULL,alpha=1,refit=FALSE,parallel=FALSE)
{
    ...
    for (k in 1:length(lambda_seq))
    {
        ...
        temp = regularized_VAR_est(Y=Y,X=X,d=d,lambda=lambda_seq[k],penalty.factor=penalty.factor,alpha=alpha,refit=FALSE,parallel=parallel);
...
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
regularied_VAR_validate = function(Y=NULL,X,d=NULL,lambda_seq=NULL,penalty.factor=NULL,alpha=alpha,
        train_size=0.8,validate_size=0.1,refit=FALSE,parallel=FALSE)
{
    ...
    dimX = ncol(X); dimY = ncol(Y); effective_n = nrow(X);
    stopifnot( nrow(X) == nrow(Y) );
    ...
    for (k in 1:length(lambda_seq))
    {
        ...
        {
            # estimation
            Y_est = Y[i:(i+train_size-1),]; X_est = X[i:(i+train_size-1),];
            temp_est = regularized_VAR_est(Y=Y,X=X,d=d,lambda=lambda_temp,penalty.factor=penalty.factor,alpha=alpha,refit=FALSE,parallel=parallel);
